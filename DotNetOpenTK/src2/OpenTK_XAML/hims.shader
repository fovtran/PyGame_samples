...
// DEF  o:GIOMG p:SGIOM q:UIGMO r:MGI s:IGJLOM t:BNO u:GMOI v:GJNLI w:GMNHNOI x:GOKMI y:GMOIUS z:GIMO ?:DA BC FI HK QQ !:BK QQ
// GHI
// JKL
// MNO
// PQR
...
#define QU(p) t(D_,A_,p) + t(A_,B_,p) + t(B_,C_,p) + t(C_,F_,p) + t(F_,I_,p) + t(I_,H_,p) + t(H_,K_,p) + t(N_,N_,p)
#define EX(p) t(B_,K_,p) + t(N_,N_,p)
//?:DA BC FI HK QQ
...
{
return ( vec2( (letterspace.x / 1.8) * .65 , 1.0-((letterspace.y / 2.) * .95) ));
}
...
d += H(r()); add(); d += STOP(r()); add(); d += I(r()); add(); d += STOP(r()); add(); d += M(r()); add();d += STOP(r()); add();d += S(r()); add();d += STOP(r()); add();  d +=EX(r());


d = clamp(d* (.65+sin(gl_FragCoord.x*PI*.5-time*4.3)*.5), 0.0, 1.0);
